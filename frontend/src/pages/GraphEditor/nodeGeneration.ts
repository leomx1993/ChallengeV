...
export function insertNodeAfterEdge<SelectedNodeName extends NodeName>({
  ...
  const localStorageValue = localStorage.getItem("nodeCondition");

  const nodesInfo = [nodes.map(({ data, position, ...rest }) => rest), [...edges]];

  let lastConditionalNodeIndex = -1;
  for(let i = nodesInfo[0].length - 1; i>=0; i--) {
    if (nodesInfo[0][i].type === 'conditional') {
      
      lastConditionalNodeIndex = i;
      break;
    }
  }

  if (lastConditionalNodeIndex !== -1) {
    nodesInfo[0][lastConditionalNodeIndex] = {
      ...nodesInfo[0][lastConditionalNodeIndex],
      "condition": localStorageValue, // Armazena o valor atual apenas no último nó condicional
    };
  }


  switch (nodeName) {
    // End nodes can't be added by Users, so they are nenodesInfover placed between two
    ...
    case "conditional": {
      ...
      console.log('nodesInfo:',nodesInfo[0])
    
      // Adicionar a chave "condition" ao objeto de dados do primeiro edge
      newEdges[0].data = { ...newEdges[0].data, condition: localStorageValue };
